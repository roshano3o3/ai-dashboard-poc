...
const DarkTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 4, opacity: 0.65, fontSize: 11 }}>
        Period {label}
      </div>
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ✅ IMPORTANT FIX:
        // "Auth session missing!" is normal when logged out -> redirect to /auth (do NOT show fatal error)
        if (!data?.user) {
          setLoading(false);
          router.replace("/auth");
          return;
        ...
        // Any other auth error -> redirect to /auth (safer than showing error)
        if (error) {
          setLoading(false);
          router.replace("/auth");
          return;
        ...
        if (dbErr) {
          setFatalError(`DB: ${dbErr.message}`);
          setLoading(false);
          return;
        }
        ...
        if (uds.length > 0) {
          setTrendDataset(uds[0]);
          setCompareDataset1(uds[0]);
          setCompareDataset2(uds[1] || uds[0]);
          setCorrDataset(uds[0]);
          setForecastDataset(uds[0]);
        }
  ...
  useEffect(() => {
    const c = getNumericColumns(trendDataset);
    if (c.length) setTrendMetric(c[0]);
  }, [trendDataset, allData]);

  useEffect(() => {
    const c = getNumericColumns(compareDataset1);
    if (c.length) setCompareMetric(c[0]);
  }, [compareDataset1, allData]);

  useEffect(() => {
    const c = getNumericColumns(corrDataset);
    if (c.length > 0) setCorrMetricX(c[0]);
    if (c.length > 1) setCorrMetricY(c[1]);
    else if (c.length === 1) setCorrMetricY(c[0]);
  }, [corrDataset, allData]);

  useEffect(() => {
    const c = getNumericColumns(forecastDataset);
    if (c.length) setForecastMetric(c[0]);
  }, [forecastDataset, allData]);

  const trendData = useMemo(() => {
    const rows = allData.filter((r) => r.dataset_name === trendDataset);
    const raw = rows
      .map((r, i) => {
        const v = toNum((r.row_data || {})[trendMetric]);
        return v !== null ? { index: i + 1, value: v } : null;
      })
    ...
    return raw.map((d, i) => {
      ...
      return {
        ...d,
        movingAvg: parseFloat(
          (slice.reduce((s, x) => s + x.value, 0) / slice.length).toFixed(2)
        ),
      };
    });
  ...
  const comparisonBarData = useMemo(() => {
    ...
    const s1 = getStats(compareDataset1);
    const s2 = getStats(compareDataset2);
    if (!s1 || !s2) return [];
  ...
  const corrData = useMemo(() => {
    ...
    return allData
      .filter((r) => r.dataset_name === corrDataset)
      .map((r) => {
        const x = toNum((r.row_data || {})[corrMetricX]);
        const y = toNum((r.row_data || {})[corrMetricY]);
  ...
  useEffect(() => {
    ...
    const sX = corrData.reduce((s, p) => s + p.x, 0);
    const sY = corrData.reduce((s, p) => s + p.y, 0);
    const sXY = corrData.reduce((s, p) => s + p.x * p.y, 0);
    const sX2 = corrData.reduce((s, p) => s + p.x ** 2, 0);
    const sY2 = corrData.reduce((s, p) => s + p.y ** 2, 0);
  ...
  const generateForecast = async () => {
    ...
    try {
      ...
      const hist = rows
        .map((r, i) => {
          const v = toNum((r.row_data || {})[forecastMetric]);
          return v !== null ? { period: i + 1, value: v } : null;
        })
        .filter(Boolean) as { period: number; value: number }[];

      if (hist.length < 3) {
        setAiInsight("⚠️ Need at least 3 records.");
      ...
      // Linear regression
      const n = hist.length;
      const sX = hist.reduce((s, d) => s + d.period, 0);
      const sY = hist.reduce((s, d) => s + d.value, 0);
      const sXY = hist.reduce((s, d) => s + d.period * d.value, 0);
      const sX2 = hist.reduce((s, d) => s + d.period ** 2, 0);
      ...
      // ✅ KEEP FEATURE: AI insight
      // ✅ SECURE IT: call our server API so Groq key is NOT in the browser
      try {
        ...
        if (out?.insight) setAiInsight("🤖 " + out.insight);
        else setAiInsight(`✅ Trend: ${slope > 0 ? "↑ increasing" : "↓ decreasing"} at ${Math.abs(slope).toFixed(2)}/period.`);
      } catch {
        setAiInsight(`✅ Trend: ${slope > 0 ? "↑ increasing" : "↓ decreasing"} at ${Math.abs(slope).toFixed(2)}/period.`);
      }
    } catch {
  ...
  if (loading)
    return (
      ...
      >
        ...
        >
          <div style={{ fontSize: 28, fontWeight: 950, color: "#E5E7EB" }}>
            Loading Analytics…
          </div>
    ...
    >
      <header style={S.header}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[
            ...
            { label: "📋 Tables", path: "/data" },
            { label: "⬆️ Upload", path: "/upload" },
          ].map(({ label, path }) => (
            <button key={path} onClick={() => router.push(path)} style={S.navBtn}>
              {label}
            </button>
          ))}
        </div>
      ...
      <main style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 20px 80px" }}>
        <div style={S.card}>
          ...
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", marginTop: 4, marginBottom: 16 }}>
        ...
        {mode === "trend" && (
          <div style={S.card}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 18, flexWrap: "wrap" }}>
              <div>
                ...
                <select value={trendDataset} onChange={(e) => setTrendDataset(e.target.value)} style={S.select}>
                  {datasets.map((d) => (
                    <option key={d} value={d} style={{ color: "#0B1220", background: "#fff" }}>
                      {d}
                    </option>
                  ))}
                </select>
              ...
              <div>
                ...
                <select value={trendMetric} onChange={(e) => setTrendMetric(e.target.value)} style={S.select}>
                  {getNumericColumns(trendDataset).map((c) => (
                    <option key={c} value={c} style={{ color: "#0B1220", background: "#fff" }}>
                      {c}
                    </option>
                  ))}
                </select>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginTop: 20 }}>
              {[
                ...
                <div key={label} style={S.statCard}>
                  <div style={{ fontSize: 11, fontWeight: 900, opacity: 0.65, textTransform: "uppercase" }}>
                    {label}
                  </div>
            ...
            <div style={{ fontSize: 11, fontWeight: 950, opacity: 0.55, textTransform: "uppercase", letterSpacing: 0.6, marginTop: 20, marginBottom: 6 }}>
              Actual + 3-Period Moving Average
            </div>
            ...
            {trendData.length > 1 && (() => {
              ...
              return (
                <div style={{ ...S.insightBox, marginTop: 16 }}>
                  ...
                  from first to last record · Moving average smooths short-term noise.
                </div>
              );
            })()}
          </div>
        ...
        {mode === "compare" && (
          <div style={S.card}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 18, flexWrap: "wrap" }}>
              {[
                { label: "Dataset 1", val: compareDataset1, set: setCompareDataset1 },
                { label: "Dataset 2", val: compareDataset2, set: setCompareDataset2 },
              ].map(({ label, val, set }) => (
                <div key={label}>
            ...
            {comparisonBarData.length > 0 && (
              <>
                ...
                <div style={{ fontSize: 11, fontWeight: 950, opacity: 0.55, textTransform: "uppercase", letterSpacing: 0.6, marginTop: 20, marginBottom: 8 }}>
                  Summary Table
                </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr>
                      {["Statistic", compareDataset1, compareDataset2, "Δ Diff"].map((h) => (
                        <th
                          key={h}
                          style={{
                            padding: "10px 12px",
                            ...
                            textAlign: h === "Statistic" ? "left" : "right",
                          }}
                        >
                          {h}
                        </th>
                  ...
                  <tbody>
                    {comparisonBarData.map((row, i) => {
                      const v1 = row[compareDataset1] as number;
                      const v2 = row[compareDataset2] as number;
                      const diff = v2 - v1;
                      return (
                        <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                          <td style={{ padding: "10px 12px", fontWeight: 800, color: "rgba(255,255,255,0.75)" }}>
                            {row.metric}
                          </td>
                          <td style={{ padding: "10px 12px", textAlign: "right", color: "#4F46E5", fontWeight: 900 }}>
                            {v1.toFixed(2)}
                          </td>
                          <td style={{ padding: "10px 12px", textAlign: "right", color: "#06B6D4", fontWeight: 900 }}>
                            {v2.toFixed(2)}
                          </td>
                          <td style={{ padding: "10px 12px", textAlign: "right", color: diff >= 0 ? "#10B981" : "#EF4444", fontWeight: 950 }}>
                            {diff >= 0 ? "+" : ""}
                            {diff.toFixed(2)}
                          </td>
                        </tr>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart>
                  ...
                  <XAxis
                    ...
                    tickLine={false}
                    label={{ value: corrMetricX, position: "insideBottom", offset: -4, fill: "rgba(255,255,255,0.55)", fontSize: 11 }}
                  />
                  <YAxis
                    dataKey="y"
                    name={corrMetricY}
                    ...
                    axisLine={false}
                    tickLine={false}
                    label={{ value: corrMetricY, angle: -90, position: "insideLeft", fill: "rgba(255,255,255,0.55)", fontSize: 11 }}
                  />
                  <Tooltip
        ...
        {mode === "forecast" && (
          <div style={S.card}>
            ...
            {aiInsight && (
              <div style={{ ...S.insightBox, marginTop: 20, borderColor: "rgba(139,92,246,0.40)", background: "rgba(139,92,246,0.10)" }}>
                {aiInsight}
              </div>
            )}
              ...
              <div style={{ height: 420, marginTop: 12 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={forecastData}>
                    ...
                    <Tooltip content={<DarkTooltip />} />
                    <ReferenceLine
                      x={forecastData.filter((d) => d.actual !== undefined).length}
                      stroke="rgba(255,255,255,0.18)"
                      strokeDasharray="4 4"
                      label={{ value: "Forecast →", fill: "rgba(255,255,255,0.45)", fontSize: 11 }}
                    />
            ...
            {forecastData.length === 0 && !forecastLoading && (
              <div style={{ height: 300, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, opacity: 0.5 }}>
...
const S: Record<string, React.CSSProperties> = {
  ...
  modeBtnActive: {
    ...
    border: "2px solid rgba(79,70,229,0.60)",
    color: "#C7D2FE",
